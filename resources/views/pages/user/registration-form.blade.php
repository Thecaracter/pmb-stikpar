                ...
                <form @submit.prevent="savePersonalData()" class="p-6">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    Gereja/Denominasi <span class="text-red-500">*</span>
                                </label>
                                <input type="text" x-model="formData.parish_name" required
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                       placeholder="Nama gereja/paroki/denominasi">
                            </div>
                        </div>
                    </div>
...
function registrationWizard() {
    return {
        // FIX: Updated currentStep initialization untuk waiting_documents
        currentStep: @if($registration->status === 'waiting_documents')
                        {{ $form->exists && $form->full_name ? '2' : '1' }}
                     @else
                        1
                     @endif,
        ...
        // Form submission methods - UPDATED
        ...
        // UPDATED goToFinalization untuk waiting_documents
        goToFinalization() {
            // For waiting_documents status, be more flexible
            if (this.registrationStatus === 'waiting_documents') {
                ...
                const isPersonalDataComplete = requiredFields.every(field => 
                    this.formData[field] && this.formData[field].trim() !== ''
                );

                if (!isPersonalDataComplete) {
                    this.showAlert('error', 'Silakan lengkapi data pribadi terlebih dahulu');
                    return;
                }
            ...
            if (missingDocs.length > 0) {
                this.showAlert('error', 'Silakan upload semua dokumen yang diperlukan terlebih dahulu');
                return;
            }
        ...
        // UPDATED canSubmit untuk waiting_documents
        canSubmit() {
        ...
        async submitFinalRegistration() {
            if (!this.canSubmit()) {
                this.showAlert('error', 'Pastikan semua data pribadi dan dokumen sudah lengkap');
                return;
            }
